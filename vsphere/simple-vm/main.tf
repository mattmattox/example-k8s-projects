...
resource "vsphere_virtual_machine" "vm" {
  ...
  extra_config = {
    "guestinfo.metadata"          = base64encode(templatefile("${path.module}/metadata.yaml", local.templatevars))
    "guestinfo.metadata.encoding" = "base64"
    "guestinfo.userdata"          = base64encode(templatefile("${path.module}/userdata.yaml", local.templatevars))
